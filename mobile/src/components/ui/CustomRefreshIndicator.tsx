...
export function CustomRefreshIndicator({ 
  ...
  const containerStyle = useAnimatedStyle(() => {
    ...
    const finalHeight = isRefreshing ? pullDistance : pullOffset;
  ...
  return (
    <AnimatedView style={[styles.container, { width }, containerStyle]}>
      <View style={styles.iconContainer}>
        ...
        {/* Main Logo (shows on pull) - Filled with black as per Zahan official branding */}
        ...
        <AnimatedView style={roundLogoStyle}>
          <Svg viewBox="0 0 478.7 478.7" width={40} height={40}>
            ...
            <Path fill="#fff" d="M275.22,167.39c-2.36,.13-4.64,.31-6.92,.37-5.64,.15-11.28,.25-16.92,.38-6.91,.16-13.78,.64-20.51,2.42-9.07,2.4-16.92,7.17-23.97,13.5-9.04,8.11-17.61,16.73-25.94,25.62-18.16,19.36-37.29,37.63-56.81,55.46-.78,.71-1.55,1.42-2.5,2.3v-92.75c0-9.1,.05-18.19-.03-27.28-.03-3.18,.72-6.07,2.3-8.71,4.55-7.58,10.75-13.12,18.81-16.18,1.5-.57,3.19-.81,4.79-.81,41.81-.04,83.61-.03,125.42-.03h1.23c.35,15.24,.69,30.42,1.04,45.72h0Z"/>
          </Svg>
        </AnimatedView>
